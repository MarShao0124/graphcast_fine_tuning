...
@hydra.main(version_base="1.2", config_path="conf", config_name="config_34var")
def main(cfg: DictConfig) -> None:
    ...
    # example; only 1 month downloaded for training
    all_years = train_years
    date_range = (
        datetime.date(min(all_years), 1, 1),
        datetime.date(max(all_years), 1, 2),
    )
    ...
    era5_xarray = xr.concat(
        [z[list(z.data_vars.keys())[0]] for z in zarr_arrays], dim="channel", join="outer"
    )

    # Remove pressure_level dimension if it exists (since pressure levels are already in channels)
    if "pressure_level" in era5_xarray.dims:
        era5_xarray_before = era5_xarray.copy()
        era5_xarray = era5_xarray.mean(dim="pressure_level")
