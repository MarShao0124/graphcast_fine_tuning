...
class ERA5Mirror:
    ...
    def __init__(self, base_path: str, fs: fsspec.AbstractFileSystem = None):
        ...
        self.metadata_file = os.path.join(self.base_path, "metadata_chunks.json")
        self.metadata = self.get_metadata()
        
        # Create coordinate metadata file for training
        self.coords_metadata_file = os.path.join(self.base_path, "metadata_coords.json")
    ...
    def create_coords_metadata(self, variables: List[Union[str, Tuple[str, int]]]):
        """Create coordinate metadata for training"""
        ...
        # Create channel names based on variables and pressure levels
        channel_names = []
        for variable, pressure_level in reformated_variables:
            if pressure_level is not None:
                channel_names.append(f"{variable}_{pressure_level}")
            else:
                channel_names.append(variable)
        
        # Create coordinate metadata structure expected by GraphCastLossFunction
        # The loss function expects to access channels by index: data_json["coords"]["channel"][index]
        # where index is an integer from channels_list
        coords_metadata = {
            "coords": {
                "channel": channel_names  # This should be a list, not a dict
            },
            "data_vars": {},
            "attrs": {},
            "dims": {
        ...
        return coords_metadata

    def save_coords_metadata(self, coords_metadata: dict):
        """Save coordinate metadata"""
        with self.fs.open(self.coords_metadata_file, "w") as f:
            json.dump(coords_metadata, f, indent=2)
    ...
    def download(
        ...
        # Generate coordinate metadata for training
        coords_metadata = self.create_coords_metadata(variables)
        self.save_coords_metadata(coords_metadata)
        print(f"Saved coordinate metadata to {self.coords_metadata_file}")
